...
// ich hab das mal geordnet ~ M√ºllmann
...
public class PlayerC implements Serializable
{
    ...
    //alle Variablen, die synchronisiert werden, m√ºssen public sein
    ...
    public void keyEvent(KeyEvent e, char type) {
        if (type == 'p'){
            ...
            //braucht eigentlich noch einen posInsideOfBounds request o.√Ñ.
            switch(e.getKeyCode()){
    ...
    // und die Methoden, die f√ºr diese Events gebraucht werden
    public void openInventory(){
        //Just for testing purpose ~unknown //Pourquoi parles-tu en anglais? ~LG // „Çà„??Áß?„?ØÁü•„Çâ„?™„?Ñ„Ä?„Çà„??„?Ç„?™„?ü„?åÁü•„?£„?¶„?Ñ„Çã ~unknown
        if (inv == null)return;
        /*
    ...
    /**
     * Gibt die obere linken Ecke (int Bl√∂cken) der Spieleransicht an
     * @param: pos: Position des Spielers relativ zur oberen linken Ecke der Sandbox
     */
    public VektorD getUpperLeftCorner(VektorD pos){
        // das -0.5 ist eine h√§ssliche L√∂sung von issue #26. Ich hab kleine Ahnung warum es geht aber es geht...
        //ist jetzt wieder rausgenommen, vorher .subtract(new VektorD(0.5,0.5)) o.ƒ.
    ...
    public void paint(Graphics g, VektorI screenSize){
        ...
        if (player.isOnline() && player.onClient()){
            if (mapIDCache!=null && mapIDCachePos!=null){
                ...
                //Zeichnen von Subsandboxen, recht √§hnlich zu dem Zeichnen der Sandbox oberhalb
